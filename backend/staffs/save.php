<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors)) {
            ...
            $checkUser = mysqli_query($conn, "SELECT * FROM staffs WHERE email = '$email'");
            ...
            if (mysqli_num_rows($checkUser) > 0) {
                ...
                // Insert record into the staffs table
                $insertQuery = "INSERT INTO staffs (first_name, last_name, gender, email, phone)
                                VALUES ('$first_name', '$last_name', '$gender', '$email', '$phone');";
    
                if ($conn->query($insertQuery) === TRUE) {
                    session_start();
                    $_SESSION['status'] = "success";
                    $_SESSION['message'] = "Staff record added successfully!";
                    session_write_close();
